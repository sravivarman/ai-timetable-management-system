...
APPROVED_LOGIN_ROLE_NAMES = {"Administrator", "Principal", "Dean", "Timetable Coordinator"}
...
class AuthenticationService:
    ...
    def create_user(self, db: Session, data: UserCreate) -> User:
        ...
        return self.users.save(db, User(email=str(data.email).lower(), full_name=data.full_name, password_hash=hash_password(data.password), roles=roles))
    ...
    def _roles_or_422(self, db: Session, role_ids: list[UUID]) -> list[Role]:
        ...
        if unsupported:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Login accounts are limited to Administrator, Principal, Dean Academics, or Timetable Coordinator")

...
def create(v:TimetableCreate,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
 scope(v);term=repo.get(db,AcademicTerm,v.academic_term_id)
...
def list_(db:Session=Depends(get_db),academic_term_id:UUID|None=None,scope_type:str|None=None,department_id:UUID|None=None,program_id:UUID|None=None,section_id:UUID|None=None,status:str|None=None,page:int=Query(1,ge=1),page_size:int=Query(20,ge=1,le=100)):return repo.page(db,Timetable,page,page_size,academic_term_id=academic_term_id,scope_type=scope_type,department_id=department_id,program_id=program_id,section_id=section_id,status=status)
...
def version(id:UUID,v:TimetableVersionCreate,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
 ...
 if run.academic_term_id!=t.academic_term_id or any(getattr(run,field)!=getattr(t,field) for field in ("scope_type","department_id","program_id","section_id")):raise HTTPException(422,"Validation run must match timetable scope and academic term")

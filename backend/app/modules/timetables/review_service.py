...
class TimetableReviewService:
 ...
 def grid(self,db,version_id,view_type,resource_id,user):
  ...
  all_entries=list(db.scalars(select(TimetableEntry).where(TimetableEntry.timetable_version_id==version_id).order_by(TimetableEntry.actual_date.asc().nulls_first(),TimetableEntry.working_day_id,TimetableEntry.period_number,TimetableEntry.id)));entries=list(all_entries)
   ...
   if laboratory:
    for period in range(entry.period_number,entry.period_number+entry.session_length):
     ...
     for other in all_entries:
      if other.laboratory_id==entry.laboratory_id and other.working_day_id==entry.working_day_id and other.actual_date==entry.actual_date and other.period_number<=period<=other.period_number+other.session_length-1:logical.setdefault(logical_capacity_key(other),entry_capacity_demand(db,other))
 ...
 def move(self,db,entry,payload,user):
  ...
  for sibling in siblings:
   old=self._entry_values(sibling);values={field:getattr(sibling,field) for field in TimetableEntryCreate.model_fields};values.update({"working_day_id":payload.working_day_id,"actual_date":payload.actual_date,"period_number":payload.period_number})
 ...
 def compare(self,db,version_id,other_id):
  ...
  def brief(row):
   value={"entry_id":str(row.id),"course_offering_id":str(row.course_offering_id),"section_id":str(row.section_id),"student_batch_id":str(row.student_batch_id) if row.student_batch_id else None,"working_day_id":str(row.working_day_id),"actual_date":row.actual_date.isoformat() if row.actual_date else None,"period_number":row.period_number,"faculty_id":str(row.faculty_id) if row.faculty_id else None,"classroom_id":str(row.classroom_id) if row.classroom_id else None,"laboratory_id":str(row.laboratory_id) if row.laboratory_id else None,"is_locked":row.is_locked}
  ...
  for key in sorted(set(left)|set(right)):
   ...
   if (a.actual_date,a.working_day_id,a.period_number)!=(b.actual_date,b.working_day_id,b.period_number):moved.append(pair)
 ...
 def transition(self,db,timetable_id,target,user,reason=None):
  ...
  if version:
   ...
   if target in {"APPROVED","PUBLISHED"}:
    ...
    session_counting_service.capture_workflow_snapshot(db,timetable,version,target,now)
 ...
 def free_resources(self,db,version_id,kind,day_id,period,section_id=None,student_batch_id=None,actual_date=None):
  version,timetable=self._context(db,version_id);day=db.get(WorkingDay,day_id)
  if not day or not day.is_active or not day.is_working_day:raise HTTPException(422,"Working day must be active")
  if version.scheduling_mode=="SLOT_BASED":
   if actual_date is None:raise HTTPException(422,"SLOT_BASED free-resource queries require actual_date")
   if not db.scalar(select(SchedulingSlotWorkingDate.id).where(SchedulingSlotWorkingDate.scheduling_slot_id==version.scheduling_slot_id,SchedulingSlotWorkingDate.working_date==actual_date,SchedulingSlotWorkingDate.is_active.is_(True))):raise HTTPException(422,"actual_date is not an active date in this Scheduling Slot")
  elif actual_date is not None:raise HTTPException(422,"WEEKLY free-resource queries must not specify actual_date")
  entries=list(db.scalars(select(TimetableEntry).where(TimetableEntry.timetable_version_id==version_id,TimetableEntry.working_day_id==day_id,TimetableEntry.actual_date==actual_date,TimetableEntry.period_number<=period,(TimetableEntry.period_number+TimetableEntry.session_length-1)>=period)))
  if kind=="faculty":
   used={x.faculty_id for x in entries if x.faculty_id};used|={x.faculty_id for x in db.scalars(select(LaboratoryFacultyAllocation).where(LaboratoryFacultyAllocation.id.in_({e.laboratory_faculty_allocation_id for e in entries if e.laboratory_faculty_allocation_id})))} if entries else set();rows=db.scalars(select(Faculty).where(Faculty.is_active.is_(True),Faculty.id.not_in(used)).order_by(Faculty.faculty_code));items=[{"id":str(x.id),"faculty_code":x.faculty_code,"full_name":x.full_name} for x in rows if availability_service.is_available(db,"FACULTY",x.id,timetable.academic_term_id,day_id,period,actual_date)]
  elif kind=="classroom":
   used={x.classroom_id for x in entries if x.classroom_id};items=[{"id":str(x.id),"room_number":x.room_number,"room_name":x.room_name} for x in db.scalars(select(Classroom).where(Classroom.is_active.is_(True),Classroom.id.not_in(used)).order_by(Classroom.room_number)) if availability_service.is_available(db,"CLASSROOM",x.id,timetable.academic_term_id,day_id,period,actual_date)]
  else:
   required=0
   if student_batch_id:
   ...
   for laboratory in db.scalars(select(Laboratory).where(Laboratory.is_active.is_(True)).order_by(Laboratory.laboratory_code)):
    if not availability_service.is_available(db,"LABORATORY",laboratory.id,timetable.academic_term_id,day_id,period,actual_date):continue
    logical={}
    for entry in entries:
  ...
  return {"version_id":version_id,"working_day_id":day_id,"actual_date":actual_date,"period_number":period,"items":items}
 ...
 def conflicts(self,db,version_id):
  ...
  for left,right in combinations(entries,2):
   if (left.actual_date,left.working_day_id)!=(right.actual_date,right.working_day_id) or left.period_number+left.session_length-1<right.period_number or right.period_number+right.session_length-1<left.period_number:continue
  ...
  for laboratory_id,laboratory in shared_labs.items():
   for actual_date,day_id in {(entry.actual_date,entry.working_day_id) for entry in entries if entry.laboratory_id==laboratory_id}:
    for period in range(1,8):
     participants=[entry for entry in entries if entry.laboratory_id==laboratory_id and entry.actual_date==actual_date and entry.working_day_id==day_id and entry.period_number<=period<=entry.period_number+entry.session_length-1];logical={}
  ...
  for entry in entries:
   ...
   for period in range(entry.period_number,entry.period_number+entry.session_length):
    ...
    key=(event_key,"FACULTY",faculty_id,entry.actual_date,entry.working_day_id,period)
    if faculty_id and key not in seen_availability and not availability_service.is_available(db,"FACULTY",faculty_id,timetable.academic_term_id,entry.working_day_id,period,entry.actual_date):add("unavailable_faculty_usage",[entry],"Faculty is scheduled in an unavailable slot")
    seen_availability.add(key)
    key=(event_key,"CLASSROOM",entry.classroom_id,entry.actual_date,entry.working_day_id,period)
    if entry.classroom_id and key not in seen_availability and not availability_service.is_available(db,"CLASSROOM",entry.classroom_id,timetable.academic_term_id,entry.working_day_id,period,entry.actual_date):add("unavailable_classroom_usage",[entry],"Classroom is scheduled outside its configured availability")
    seen_availability.add(key)
    if entry.laboratory_id:
     laboratory=db.get(Laboratory,entry.laboratory_id)
     key=(event_key,"LABORATORY",entry.laboratory_id,entry.actual_date,entry.working_day_id,period)
     if laboratory and key not in seen_availability and not availability_service.is_available(db,"LABORATORY",laboratory.id,timetable.academic_term_id,entry.working_day_id,period,entry.actual_date):add("blocked_laboratory_usage",[entry],"Laboratory is scheduled outside its configured availability")
     seen_availability.add(key)

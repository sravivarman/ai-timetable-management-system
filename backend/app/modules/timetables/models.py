...
from sqlalchemy import Boolean,CheckConstraint,Date,DateTime,ForeignKey,Integer,String,JSON,UniqueConstraint,func
from sqlalchemy.orm import Mapped,mapped_column
from sqlalchemy.types import Uuid
from app.db.base import Base
class Timetable(Base):
 __tablename__="timetables";__table_args__=(CheckConstraint("(scheduling_mode = 'WEEKLY' AND scheduling_slot_id IS NULL) OR (scheduling_mode = 'SLOT_BASED' AND scheduling_slot_id IS NOT NULL)",name="ck_timetable_scheduling_mode_slot"),)
 id:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),primary_key=True,default=uuid4);academic_term_id:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),ForeignKey("academic_terms.id"));scope_type:Mapped[str]=mapped_column(String(20));department_id:Mapped[UUID|None]=mapped_column(Uuid(as_uuid=True));program_id:Mapped[UUID|None]=mapped_column(Uuid(as_uuid=True));section_id:Mapped[UUID|None]=mapped_column(Uuid(as_uuid=True));scheduling_mode:Mapped[str]=mapped_column(String(20),default="WEEKLY",nullable=False,index=True);scheduling_slot_id:Mapped[UUID|None]=mapped_column(Uuid(as_uuid=True),ForeignKey("scheduling_slots.id",ondelete="RESTRICT"),index=True);name:Mapped[str]=mapped_column(String(255));status:Mapped[str]=mapped_column(String(20),default="DRAFT");active_version_id:Mapped[UUID|None]=mapped_column(Uuid(as_uuid=True));created_by:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),ForeignKey("users.id"));created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),server_default=func.now(),nullable=False);updated_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),server_default=func.now(),onupdate=func.now(),nullable=False)
class TimetableVersion(Base):
 __tablename__="timetable_versions";__table_args__=(UniqueConstraint("timetable_id","version_number",name="uq_timetable_version_number"),CheckConstraint("(scheduling_mode = 'WEEKLY' AND scheduling_slot_id IS NULL) OR (scheduling_mode = 'SLOT_BASED' AND scheduling_slot_id IS NOT NULL)",name="ck_timetable_version_scheduling_mode_slot"))
 id:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),primary_key=True,default=uuid4);timetable_id:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),ForeignKey("timetables.id"));version_number:Mapped[int]=mapped_column(Integer);version_name:Mapped[str|None]=mapped_column(String(255));source_type:Mapped[str]=mapped_column(String(20));validation_run_id:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),ForeignKey("validation_runs.id"));scheduling_mode:Mapped[str]=mapped_column(String(20),default="WEEKLY",nullable=False,index=True);scheduling_slot_id:Mapped[UUID|None]=mapped_column(Uuid(as_uuid=True),ForeignKey("scheduling_slots.id",ondelete="RESTRICT"),index=True);solver_status:Mapped[str]=mapped_column(String(20),default="NOT_STARTED");is_active:Mapped[bool]=mapped_column(Boolean,default=True);is_locked:Mapped[bool]=mapped_column(Boolean,default=False);created_by:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),ForeignKey("users.id"));created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),server_default=func.now(),nullable=False);updated_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),server_default=func.now(),onupdate=func.now(),nullable=False)
...
class TimetableSessionProgressSnapshot(Base):
 __tablename__="timetable_session_progress_snapshots"
 __table_args__=(UniqueConstraint("timetable_version_id","workflow_status","course_offering_id",name="uq_session_progress_snapshot"),)
 id:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),primary_key=True,default=uuid4)
 timetable_id:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),ForeignKey("timetables.id",ondelete="CASCADE"),nullable=False,index=True)
 timetable_version_id:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),ForeignKey("timetable_versions.id",ondelete="CASCADE"),nullable=False,index=True)
 course_offering_id:Mapped[UUID]=mapped_column(Uuid(as_uuid=True),ForeignKey("course_offerings.id",ondelete="RESTRICT"),nullable=False,index=True)
 workflow_status:Mapped[str]=mapped_column(String(20),nullable=False,index=True)
 scheduled_sessions:Mapped[int]=mapped_column(Integer,nullable=False)
 created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),server_default=func.now(),nullable=False)

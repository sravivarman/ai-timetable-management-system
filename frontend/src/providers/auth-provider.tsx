...
interface AuthContextValue { user: User | null; loading: boolean; login(email: string, password: string): Promise<void>; logout(): Promise<void>; hasRole(...roles: string[]): boolean }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string) => { tokenStore.set(await authApi.login(email, password)); setUser(await authApi.me()); router.replace("/dashboard") }, [router]);

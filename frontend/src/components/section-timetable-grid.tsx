...
export function SectionTimetableGrid({ grid, printable = true }: { grid: TimetableGrid; printable?: boolean }) {
  ...
  return <div className="timetable-visualization">
    ...
    <div className={clsx("overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700", printable && "print:overflow-visible print:border-0")} tabIndex={0} aria-label="Scrollable weekly timetable">
      <table className="weekly-grid w-full min-w-[1050px] table-fixed border-collapse text-sm print:min-w-0"><thead><tr className="bg-slate-100 dark:bg-slate-800"><th className="sticky left-0 top-0 z-20 w-28 border-b border-r bg-slate-100 p-3 text-left dark:border-slate-700 dark:bg-slate-800">Day</th>{Array.from({ length: 7 }, (_, index) => <th key={index} className="sticky top-0 z-10 border-b border-r bg-slate-100 p-3 text-center dark:border-slate-700 dark:bg-slate-800">Period {index + 1}</th>)}</tr></thead><tbody>{grid.days.map((day) => {
        ...
        return <tr key={day.working_day_id}><th className="sticky left-0 z-10 border-b border-r bg-slate-50 p-3 text-left align-top font-semibold dark:border-slate-700 dark:bg-slate-800">{day.day_name}</th>{cells}</tr>;
      })}</tbody></table>
    </div>
  </div>;
}
...
function EntryCard({ entry }: { entry: GridEntry }) {
  ...
  return <article tabIndex={0} aria-label={`${entry.course_code} ${entry.course_name}, ${entry.day_name}, periods ${entry.period_numbers.join(", ")}`} className={clsx("group relative min-h-24 rounded-lg border p-2.5 shadow-sm", tones[entry.course_type] ?? "border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800", entry.is_locked && "ring-2 ring-violet-500", entry.is_manual && "border-dashed border-slate-600")}>
    <div className="flex items-start justify-between gap-2"><strong className="text-xs">{entry.course_code}</strong><span className="flex gap-1">{entry.is_manual && <Pencil aria-label="Manual entry" className="h-3.5 w-3.5" />}{entry.is_locked && <Lock aria-label="Locked entry" className="h-3.5 w-3.5 text-violet-700" />}</span></div>{combinedSections && <p className="mt-1 rounded bg-indigo-100 px-1.5 py-0.5 text-[10px] font-semibold text-indigo-800 dark:bg-indigo-950 dark:text-indigo-200">Combined: {combinedSections}</p>}<p className="mt-1 line-clamp-2 text-xs text-slate-700 dark:text-slate-200">{entry.course_name}</p><p className="mt-2 text-[11px] text-slate-600 dark:text-slate-300">{entry.faculty_code ?? "No faculty"}{facility && ` · ${facility}`}{entry.batch_name && ` · ${entry.batch_name}`}</p>
    <div role="tooltip" className="pointer-events-none absolute left-1/2 top-full z-30 mt-2 hidden w-72 -translate-x-1/2 rounded-lg bg-slate-950 p-3 text-left text-xs text-white shadow-xl group-hover:block group-focus:block print:hidden"><p className="font-semibold">{entry.course_code} · {entry.course_name}</p><dl className="mt-2 grid grid-cols-[auto,1fr] gap-x-2 gap-y-1"><dt>Type</dt><dd>{entry.course_type.replaceAll("_", " ")}</dd><dt>{combinedSections ? "Sections" : "Section"}</dt><dd>{combinedSections ?? entry.section_code}</dd>{entry.combined_teaching_group_code && <><dt>Common class</dt><dd>{entry.combined_teaching_group_code}</dd></>}<dt>Faculty</dt><dd>{[entry.faculty_code, entry.faculty_name].filter(Boolean).join(" · ") || "Not assigned"}</dd><dt>Periods</dt><dd>{entry.period_numbers.join(", ")} ({entry.start_time}–{entry.end_time})</dd><dt>Facility</dt><dd>{facility ?? "Not assigned"}</dd><dt>Student group</dt><dd>{entry.batch_name ?? "Full section"}</dd><dt>Status</dt><dd><StatusBadge value={entry.entry_status} /></dd></dl></div>
  </article>;
}

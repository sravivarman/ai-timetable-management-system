...
export function MasterRecordForm({ config, initial, lookupRecords, mode, busy, onClose, onSubmit }: { config: MasterConfig; initial?: MasterRecord; lookupRecords: Record<string, MasterRecord[]>; mode: "create" | "edit" | "duplicate" | "bulk"; busy: boolean; onClose(): void; onSubmit(payload: Record<string, unknown>): void }) {
  ...
  const update = (field: MasterField, value: FormValue) => { setValues((current) => {
    ...
    if (config.slug === "course-offerings" && field.name === "laboratory_selection_mode") { if (["AUTO", "RESTRICTED"].includes(String(value))) next.laboratory_override_id = ""; if (value !== "RESTRICTED") next.allowed_laboratory_ids = []; }
    if (config.slug === "course-offerings" && field.name === "course_id") { next.laboratory_override_id = ""; next.allowed_laboratory_ids = []; next.laboratory_selection_mode = "AUTO"; }
    return next;
...
function FormField({ field, value, error, lookupRecords, partial, onChange }: { field: MasterField; value: FormValue | undefined; error?: string; lookupRecords: Record<string, MasterRecord[]>; partial: boolean; onChange(value: FormValue): void }) {
  ...
  if (field.name === "laboratory_selection_mode") { const copy = { AUTO: ["Automatic selection", "The solver may use any course-eligible laboratory."], PREFERRED: ["Prefer a laboratory", "The solver prefers one laboratory but may use another eligible one."], RESTRICTED: ["Restrict to selected laboratories", "The solver may use only the selected offering-specific subset."], FIXED: ["Require a laboratory", "The solver must use exactly the selected laboratory."] } as const; return <fieldset><legend className="label">{label}</legend><div className="space-y-2 rounded-lg border p-3 dark:border-slate-700">{(["AUTO", "PREFERRED", "RESTRICTED", "FIXED"] as const).map((option) => <label key={option} className="flex items-start gap-2 text-sm"><input type="radio" name={field.name} value={option} checked={value === option} onChange={() => onChange(option)} /><span><strong>{copy[option][0]}</strong><span className="block text-xs text-slate-500">{copy[option][1]}</span></span></label>)}</div>{help}{error && <p className="mt-1 text-xs text-red-700">{error}</p>}</fieldset>; }
...
function isFormVisible(config: MasterConfig, field: MasterField, values: Record<string, FormValue>, lookups: Record<string, MasterRecord[]>) {
  ...
  if (config.slug === "course-offerings" && ["laboratory_selection_mode", "laboratory_override_id", "allowed_laboratory_ids"].includes(field.name)) {
...
function validate(config: MasterConfig, values: Record<string, FormValue>, only?: Set<string>, lookupRecords: Record<string, MasterRecord[]> = {}) {
  ...
  if (config.slug === "laboratories" && values.concurrent_usage_mode === "CAPACITY_SHARED" && get("capacity") < 1) errors.capacity = "Capacity is required and must be greater than zero for capacity-shared facilities.";
  ...
  if (config.slug === "course-offerings") { const course = (lookupRecords["/courses"] ?? []).find((row) => row.id === String(values.course_id ?? "")); const mode = String(values.laboratory_selection_mode || "AUTO"); if (isLaboratoryCapableCourse(course) && ["PREFERRED", "FIXED"].includes(mode) && !values.laboratory_override_id) errors.laboratory_override_id = `${mode === "FIXED" ? "Required" : "Preferred"} laboratory is required.`; if (isLaboratoryCapableCourse(course) && mode === "RESTRICTED" && (!Array.isArray(values.allowed_laboratory_ids) || !values.allowed_laboratory_ids.length)) errors.allowed_laboratory_ids = "Select at least one allowed laboratory."; }
...
function filteredLookups(config: MasterConfig, field: MasterField, values: Record<string, FormValue>, lookupRecords: Record<string, MasterRecord[]>) {
  ...
  if (config.slug === "course-offerings" && ["laboratory_override_id", "allowed_laboratory_ids"].includes(field.name)) {

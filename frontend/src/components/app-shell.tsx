...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const isReportViewer = roles.includes("REPORT_VIEWER");
  const visible = isReportViewer ? items.filter((item) => item.label === "Reports") : items.filter((item) => !item.roles || item.roles.some((role) => roles.includes(role)));
  ...
  return <div className="min-h-screen bg-slate-50 dark:bg-slate-950 dark:text-slate-100">
    ...
    <aside aria-label="Primary navigation" className={clsx("fixed inset-y-0 left-0 z-40 w-64 border-r border-slate-800 bg-slate-950 text-white transition-transform lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}>
      ...
      <nav className="space-y-1 p-3" aria-label="Main navigation">{visible.map(({ label, href, icon: Icon }) => { const target = isReportViewer && label === "Reports" ? "/reports?report=administrative-faculty_master" : href; return <Link key={href} href={target} onClick={() => setOpen(false)} className={clsx("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm", pathname.startsWith(href) ? "bg-brand-600 font-semibold" : "text-slate-300 hover:bg-slate-800 hover:text-white")}><Icon className="h-4 w-4" />{label}</Link>; })}</nav>
    </aside>
    <div className="lg:pl-64"><header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-900/95 sm:px-6 print:hidden"><button aria-label="Open navigation" aria-expanded={open} className="rounded-lg p-2 hover:bg-slate-100 dark:hover:bg-slate-800 lg:hidden" onClick={() => setOpen(true)}><Menu /></button><p className="hidden min-w-0 truncate font-semibold text-brand-900 dark:text-slate-100 xl:block">AI-Based Engineering College Timetable Management System</p><div className="ml-auto flex items-center gap-2">{!isReportViewer && <GlobalSearch />}<button className="button-secondary h-9 px-2" aria-label={`Use ${dark ? "light" : "dark"} theme`} onClick={toggleTheme}>{dark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</button><div className="hidden text-right sm:block"><p className="text-sm font-medium">{user?.full_name}</p><p className="max-w-52 truncate text-xs text-slate-500">{roles.map((role) => role === "REPORT_VIEWER" ? "Report Viewer" : role).join(", ")}</p></div><button className="button-secondary h-9 gap-2" onClick={() => void logout()}><LogOut className="h-4 w-4" /><span className="hidden md:inline">Logout</span></button></div></header><main className="mx-auto max-w-[1500px] p-4 sm:p-6"><Breadcrumbs />{children}</main></div>
  </div>;
}
